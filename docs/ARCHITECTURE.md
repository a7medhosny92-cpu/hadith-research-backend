...
| 7 | build graph | `scripts.build_graph` | **full** | `narrators.db`, `muhmal.json` |
...
- `lookup(name) → RijalMatch | None` (one best answer); `candidates(name)` returns the **full
  homonym set** (capped at 40 — a bare ism with dozens of bearers is too generic to resolve). The
  pivotal field **`grade_agreed = all tied candidates share one category`**.
...
### `tahdhib.py` — al-Mizzī's authoritative company
`tahdhib_associations(records, rijal)` turns each تهذيب الكمال tarjama into an association (رجال
canonical name → tokens of his شيوخ+تلاميذ) **only when the man resolves unambiguously**.
...
`analyze_isnad(text, rijal=None, canon=None, muhmal=None)` first **splits the chain** (transmission
verbs; «ح» = a route seam; «أنّ» opens a report) — dropping what is never a narrator: a back-reference
«بهذا الإسناد», a hadith-number «م - ٢٣٤٥», a lone ramz letter, and action verbs that open a matn
(«يخطب/يحدّث…», unless a transmission verb follows) [#117]. A **تحويل (ح)** is recorded as a *route
seam*: the men either side of it are not a real link and are not used as each other's company. Then,
when given the rijal layer, it grades each link via the **resolution ladder**:
...
4. **`rijal.lookup(name)`** → `RijalMatch`.
5. **تمييز بالطبقة (position rule).** Only when the chain **reaches the Prophet ﷺ** is the terminal link
   ...
   صحابي match **deep** (≤ terminal−2) prefers a non-صحابي homonym (anachronism), while the **penultimate**
   link keeps صحابي عن صحابي.
...
**The central problem and the right data.** The bulk of audit flags are **A («مشترك»)** — a name
that matches more than one man. The key realisation: a **grade is not an identity signal** — it says
how good a man is, not which man he is. Using grade-agreement to "handle" ambiguity is a *safety
patch* (don't get the vote wrong); for true homonyms with different grades it just gives up
(يُتوقَّف). Identity is fixed by **who sits around the man in the chain**. Three signals, in order of
strength:

1. **Corpus redundancy — تمييز المهمل بالمسمّى.** *Status: **DONE** — PR #110, `app/rijal/muhmal.py`.*
   The same link is bare in one chain and full in another → the corpus disambiguates itself,
   deterministically, with zero downloads — the classical method run over ~90k chains. Measured on a
   **partial** corpus (~10k chains): 1309 context maps, ~981 resolutions. On the full corpus
   (~9× redundancy) the yield is expected far higher, and resolving one high-frequency narrator
   (غندر، ابن مهدي، الهمداني…) dissolves thousands of «مشترك» at once.

2. **طبقة / death-year (the generational constraint).** *Status: **TODO** — the natural complement for
   the ~350 cases redundancy can't confirm.* A man's شيخ dies before him, his تلميذ after — so the
   chain neighbours place him in time and exclude homonyms of another era. Sources: الثقات (ابن
   حبان), تاريخ الإسلام (الذهبي). **Gated on the new W/S/A** — decide after the rebuild whether it's
   still needed.

3. **التخريج alignment (parallel routes).** *Status: **FUTURE**.* The same ḥadīth runs through several
   collections; aligning the chains position-by-position transfers a full name onto a bare one — it's
   signal #1 lifted to the whole-ḥadīth level.

Alongside identity, **dedup (#109)** is the parallel lever for **duplicates** (the same man written
two ways ≈ half of A). **Grade-agreement** remains only as the safety net.

**Merged on main (the arc #84 → #110):** grade-agreement gate; leading-run / ancestor-in-nasab;
teknonym reverse-only; prefix preference; single-token & bare-grave junk drops (#89); the
**matn-completeness fix (#102)**; the **تهذيب الكمال extractor + graph wiring (#103/#104)**;
build_graph perf (#106); **same-man dedup (#109)**; **تمييز المهمل (#110)**.

**Last known audit:** **W 833 / S 5783 / A 39,312** (89,520 chains · 10,519 rijal) — the
post-matn-fix + تهذيب numbers (#108, A −969 vs the prior run). **#109/#110 are merged but not yet
measured** on a full-corpus rebuild.

**Pending:** the user runs `update.bat` to completion (possibly **×2** for the graph lag) → reports
the new **W/S/A** from the «التدقيق» tab — the real test of how much identity data (dedup + corpus
redundancy) drops A. Then decide on signal (2). Also pending: download الجرح والتعديل 2170 on the
user's machine; ROADMAP item 2 (double-opinion) and item 8b (multi-critic verdicts as a rich rijal
source).

**Docs currently behind the code (to reconcile, not changed here):** `CLAUDE.md` "Current work"
(branch/repo line stale; says dedup "not on main" though #109 merged; #110 unmentioned); `README.md`
(the "never duplicating → no false مشترك" dedup claim is optimistic; doesn't mention the audit step);
`ROADMAP.md` (no DONE markers — item 1 is effectively done, item 8 partially); `AUDIT.md` (a frozen
2026-06-06 snapshot with no per-finding status — cross-reference `CLAUDE.md` for fixed-vs-open); the
UI's "audit not built" hint still points to `audit.bat` instead of `update.bat` (≈ `index.html:1295`).
