...
spelled-out in تقريب). **Four junk drops** (these would *contain-match* and mis-grade fuller
...
**sorted folded token set** (order-independent for node identity, unlike the index). Built by
`add_chain` aggregating every adjacent تلميذ→شيخ link.
...
`analyze_isnad(text, rijal=None, canon=None, muhmal=None)` parses the chain and (when given the
rijal layer) grades it. **Per-narrator resolution ladder** (verified at `isnad.py:205-231`):
...
3. **`canon.canonical(name, context)`** — if still مهمل, company disambiguation over the homonym
   set (`context` = all other narrators' tokens).
4. **`rijal.lookup(name)`** → `RijalMatch`.
5. **The grade-agreement gate:**
   ```python
   ...
   **held (يُتوقَّف), never graded weak**. When the tied candidates **agree** (عدي بن حاتم → both
   صحابي; الليث → both ثقة) the grade is used. **The card still shows all candidates either way.**
