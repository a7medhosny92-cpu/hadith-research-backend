...
class Settings(BaseSettings):
    ...
    # no matter what local/remote are set to. Default: the LOCAL qwen2.5:3b — free, offline, fast (a
    # direct-answer instruct model, NO chain-of-thought, ideal for the big idempotent batch) and with
    # NO weekly cap (the Ollama-Cloud free tier limits the whole account). build_rijal_llm also strips
    # a <think>…</think> block, so a reasoning model (qwen3.5:4b…) works too — slower per call though.
    # Override in .env (e.g. LLM_EXTRACT_MODEL=ollama/gemma4:31b-cloud for the cloud extractor).
    llm_extract_model: str = "ollama/qwen2.5:3b"
