...
# Folded leaf-heading words: a «باب/فصل» nests one notch UNDER a same-level grouping («كتاب», «جماع
# أبواب», «أبواب») — otherwise turath's equal levels make a باب REPLACE the grouping at that level,
# orphaning it (ابن خزيمة's «جماع أبواب X ← باب Y» hierarchy, flattened to «كتاب ← باب Y»).
_LEAF_HEAD = {normalize_for_search(w) for w in ("باب", "فصل")}


def _eff_level(level: int, title: str) -> float:
    """Effective heading depth: a «باب/فصل» is half a level DEEPER than a same-level grouping, so the
    grouping (كتاب/جماع أبواب/أبواب) stays its parent. Only relative order matters to the active path,
    so this never changes books where كتاب/باب already have distinct levels."""
    head = normalize_for_search(title).split()
    return level + (0.5 if head and head[0] in _LEAF_HEAD else 0.0)
